...
def Generate_GW_data(nbr_file, distance_min, distance_max, m1_min, m1_max, m2_min, m2_max, f_lower, freq, theta_min, theta_max, phi_min, phi_max):


	print("############### data generation #####################")
	tab_waveform = []
	for i in tqdm(range(0, nbr_file)):
		waveform = []
...
for run in range(0,nbr_run):
	...
	if run==0:
		...
		tab_waveform = Generate_GW_data(nbr_file,
						distance_min, 
						distance_max, 
						m1_min, 
						m1_max, 
	...
	seed = run + nbr_run * nbr_initial_file # to be sure is different each time
	proba = 0.5
	...
	# inject into noise 
	seed = run + nbr_run * nbr_initial_file # to be sure is different each time
	tab_noisyGW, tab_snr = fake_noisyGW(tab_killGW, seed, run, freq)
